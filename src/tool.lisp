...
(defun run-tests (test-cmd)
  ...
  (multiple-value-bind (output err code)
    ...
    (if (> code 0)
        (let* ((out (format nil "~A" (if (string= "" output) err output)))
               (lines (cl-ppcre:split "\\n" out))
               (line-count (length lines)))
          `(:status :error :details ,(subseq lines (max 0 (- line-count 1000)))))
...
are stuck in a loop and are repeating. Can be between 0 and 100. 0 means loop is not detected, 100 means loop is definitely detected.")))
...
(defmethod tool-execute ((tool loop-detection-tool) args &rest options)
  ...
  (let* ((certainty-raw (aget args :loop-detected-percentage))
         (certainty (if (stringp certainty-raw)
                        (parse-integer certainty-raw)
                        certainty-raw)))
